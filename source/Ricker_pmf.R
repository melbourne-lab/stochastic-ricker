# Probability mass functions (pmf) for stochastic Ricker models
...
# The integral uses Eulers-McLaurin trapezoid geometry on P[R=r] - i.e. equal
...
#
# This version has an automatic stopping rule. With the default eps and ptol
# settings this ends up always stopping at n=6. It is slower than version 1 due
# to looping. The integral uses Eulers-McLaurin trapezoid geometry on P[R=r] -
# i.e. equal increments on the probability scale. This effectively gives us an
...
# DiffvarD_LAint.r, which in turn follows Numerical Recipes in C, trapzd.c and
# qtrap.c. It is a clear candidate for rewriting in C because the integral is
# calculated separately for each cell in the vector Ntp1. The ptol and eps
# values are relatively imprecise but sufficient for model fitting.
# 
...
# ---ZIP-binomial pmf for Ricker model
#
# This pmf is a zero-inflated variant of the Poisson-binomial model that allows
# for the case when all individuals at the beginning of a generation are female
# and do not have access to males to mate with. Simple mod to Poisson-binomial:
# sum over females - 1, instead of total females.
# 
Ricker_dzipbinom <- function(Ntp1,Nt,p,R,alpha,log=FALSE){
  sump <- rep(NA,length(Ntp1)) #Sum of the probabilities
  for (i in 1:length(Ntp1)){
    if (Nt[i] %in% c(0,1)) { #If Nt=0 or 1, then probability of any offspring = 0
      sump[i] <- ifelse(Ntp1[i]==0,1,0) #Pr[Ntp1=0]=1, Pr[Ntp1>0]=0
      next
    }
    f <- 0:(Nt[i]-1) #f is number of females. If f=Nt, probability of any offspring = 0
    sump[i] <- sum( exp( dbinom(f,Nt[i],p,log=TRUE) + 
                       dpois(Ntp1[i],(1/p)*R*f*exp(-alpha*Nt[i]),log=TRUE) ) )
    #We use the log scale to ensure small probs multiply accurately
    #Add the case for f=Nt, Ntp1=0
    sump[i] <- sump[i] + ifelse( Ntp1[i]==0, dbinom(Nt[i],Nt[i],p), 0 )
  }
  if (log==FALSE) {
    return( sump )
  } #else
...
#----ZINB-binomial (demographic) pmf for Ricker model
#
# This pmf is a zero-inflated variant of the Negative-binomial-binomial
# (demographic) model that allows for the case when all individuals at the
# beginning of a generation are female and do not have access to males to mate
# with. Simple mod to negative binomial-binomial (demographic): sum over 
# females - 1, instead of total females.
# 
Ricker_dzinbbinom_d <- function(Ntp1,Nt,p,R,alpha,k,log=FALSE){
  sump <- rep(NA,length(Ntp1)) #Sum of the probabilities
  for (i in 1:length(Ntp1)){
    if (Nt[i] %in% c(0,1)) { #If Nt=0 or 1, then probability of any offspring = 0
      sump[i] <- ifelse(Ntp1[i]==0,1,0) #Pr[Ntp1=0]=1, Pr[Ntp1>0]=0
...
#----ZINB-binomial (environmental) pmf for Ricker model
#
# This pmf is a zero-inflated variant of the Negative-binomial-binomial
# (environmental) model that allows for the case when all individuals at the
# beginning of a generation are female and do not have access to males to mate
# with. Simple mod to negative binomial-binomial (environmental): sum over 
# females - 1, instead of total females.
#
Ricker_dzinbbinom_e <- function(Ntp1,Nt,p,R,alpha,k,log=FALSE){
  sump <- rep(NA,length(Ntp1)) #Sum of the probabilities
  for (i in 1:length(Ntp1)){
    if (Nt[i] %in% c(0,1)) { #If Nt=0 or 1, then probability of any offspring = 0
      sump[i] <- ifelse(Ntp1[i]==0,1,0) #Pr[Ntp1=0]=1, Pr[Ntp1>0]=0
...
# --ZINB-binomial-gamma pmf for Ricker model
#
# This pmf is a zero-inflated variant of the Negative-binomial-binomial-gamma
# model that allows for the case when all individuals at the beginning of a
# generation are female and do not have access to males to mate with. Algorithm
# is the same as for Negative binomial-binomial-gamma but we are integrating
# over the zero-inflated-negative-binomial-binomial-demographic model, which
# sums over females - 1, instead of total females.
# 
# Requires a standard number of increments and has no automatic resolution
# setting. Because of this, the whole vector of Ntp1 can be done at once and it
# is faster. See notes for Ricker_dnbinomgamma if higher accuracy is needed. The
...
# values are quite course but sufficient for model fitting.
# 
Ricker_dzinbbinomgamma <- function(Ntp1,Nt,p,R,alpha,kD,kE,log=FALSE,ptol=1e-3,nmax=6) {
# Integrate over R using an extended trapezoid rule
# Set initial increments
  Rl <- qgamma(ptol, shape = kE, scale = R / kE )
  Ru <- qgamma(1-ptol, shape = kE, scale = R / kE )
  Rhalf <- qgamma(0.5, shape = kE, scale = R / kE )
# One whole increment first and the halfway point
  psum <- 0.5 * ( Ricker_dzinbbinom_d(Ntp1,Nt,p,Rl,alpha,kD)
                  + Ricker_dzinbbinom_d(Ntp1,Nt,p,Ru,alpha,kD) )
  psum <- psum + Ricker_dzinbbinom_d(Ntp1,Nt,p,Rhalf,alpha,kD)
# Progressively smaller increments (halving size each time)
  for (n in 2:nmax) {
    totincs <- 2^n                #The total number of increments (old + new)
    newpts <- 2^(n-1)             #The number of new points
    ...
    pmids <- 0                    #Initialize to zero
    for (r in R_evals) {
      pmids <- pmids + Ricker_dzinbbinom_d(Ntp1,Nt,p,r,alpha,kD)
    }
    psum <- psum + pmids          #Add the new points to previous ones
    pinc <- (1-2*ptol)/totincs    #Current increment size on probability scale
    pint <- psum * pinc           #Current approximation for p
