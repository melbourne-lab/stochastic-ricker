# Likelihoods for stochastic Ricker models
# These return negative log likelihoods (nll)
# The .nll functions require the global variables Nt and Ntp1

source("source/Ricker_pmf.r") #For the pmfs (not used for first three .nlls)
...
#----Likelihood for the ZIP-binomial Ricker model
#
Ricker_zipbinom.nll <- function(lnR,lnalpha){
  #Probability of the data
  -sum(Ricker_dzipbinom(Ntp1,Nt,0.5,exp(lnR),exp(lnalpha),log=TRUE))
}

#----Likelihood for the ZINB-binomial (demographic) Ricker model
#
Ricker_zinbbinom_d.nll <- function(lnR,lnalpha,lnk){
  #Probability of the data
  -sum(Ricker_dzinbbinom_d(Ntp1,Nt,0.5,exp(lnR),exp(lnalpha),exp(lnk),log=TRUE))
}

#----Likelihood for the ZINB-binomial (environmental) Ricker model
#
Ricker_zinbbinom_e.nll <- function(lnR,lnalpha,lnk){
  #Probability of the data
  -sum(Ricker_dzinbbinom_e(Ntp1,Nt,0.5,exp(lnR),exp(lnalpha),exp(lnk),log=TRUE))
}

#----Likelihood for the ZINB-binomial-gamma Ricker model
#
Ricker_zinbbinomgamma.nll <- function(lnR,lnalpha,lnkD,lnkE){
  #Probability of the data
  -sum(Ricker_dzinbbinomgamma(Ntp1,Nt,0.5,exp(lnR),exp(lnalpha),
                                          exp(lnkD),exp(lnkE),log=TRUE))
}
